...
/// \fn BTwrite
/// \brief write float parameter to BT serial
/// 
void BTwrite(float value){
    char outstr[21];
    dtostrf(value, 20, 12, outstr);
    BTLEserial.print(outstr);
}

/// \fn BTSend
/// \brief send data to BT serial
/// 
void BTSend(){
  ...
  if (status == ACI_EVT_CONNECTED) {
    ...
    BTwrite(gps.venus838data_raw.velocity*3.6);
  }
};
